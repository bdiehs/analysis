...
Lemma nbhs_fin_out_above (r : R) (e : {posnum R}) (A : set {ereal R}) :
  (fun y => `|contract r%:E - contract y| < e%:num) `<=` A ->
...
- rewrite {1}/nbhs /= /ereal_nbhs.
  ...
  + exists (diag (1 - contract M%:E)); rewrite /diag.
...
Lemma closed_ereal_le_ereal y : closed (fun x => y <= x)%E.
Proof.
...
Lemma closed_ereal_ge_ereal y : closed (fun x => y >= x)%E.
Proof.
rewrite (_ : [set x | y >= x]%E = ~` [set x | y < x]%E); last first.
  by rewrite predeqE=> x; split=> [rx|/negP]; [apply/negP|]; rewrite -leNgt.
